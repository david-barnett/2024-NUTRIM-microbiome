...
```{r}
tt_get(ps) %>% as.data.frame() %>% filter(is.na(Genus)) %>% head()
```
...
-   To look at and fix the taxonomy table interactively, try `tax_fix_interactive(ps)` in the console (You may need to allow popups on your browser!)
