...
2.  **Secondary aims** (see part 2B):

    a.  **Activity:** Is current disease activity level associated with microbiota diversity, composition, or the relative abundance of specific taxa?

    b.  **Medication:** Are IBD-related medications associated with microbiota diversity, composition, or the relative abundance of specific taxa?
...
3.  **Triangle** **inequality**: For any samples $a$, $b$, and $c$, $d(a, c) ≤ d(a, b) + d(b, c)$
    ...
    -   \(3\) is not true for e.g. Bray-Curtis, but in practice it is very rarely problematic.
:::
...
-   Your methodological choices matter, you should report what you did:
    ...
    -   any covariates included in the statistical model (see next note)
